  ...
  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        background: `linear-gradient(${gradientAngle}deg, hsl(${hueShift}, 70%, 15%), hsl(${hueShift + 60}, 60%, 10%))`,
      }}
    />
  );
};
...
Use a Lucide React component for `icon`; never pass emoji strings.
  ...
  return (
    <div className="flex flex-wrap">
      {text.split("").map((char, index) => {
        const charFrame = frame - index * charDelay;
